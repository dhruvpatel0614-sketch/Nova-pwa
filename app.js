...
function device(){let a=[`Online: ${navigator.onLine?"yes":"no"}`,`Screen: ${screen.width}×${screen.height}`];if(navigator.getBattery)navigator.getBattery().then(b=>$("#device").innerHTML=a.join("<br>")+`<br>Battery: ${Math.round(b.level*100)}%<br>Charging: ${b.charging?"yes":"no"}`);else $("#device").innerHTML=a.join("<br>")}
function locate(){if(!navigator.geolocation){$("#location").textContent="GPS unavailable.";return}$("#location").textContent="Requesting GPS…";navigator.geolocation.getCurrentPosition(p=>{let lat=p.coords.latitude.toFixed(6),lon=p.coords.longitude.toFixed(6);$("#location").innerHTML=`Latitude: ${lat}<br>Longitude: ${lon}<br>Accuracy: ±${Math.round(p.coords.accuracy)}m`;localStorage.novaLocation=JSON.stringify({lat,lon,time:Date.now()})},e=>$("#location").textContent=e.message,{enableHighAccuracy:true,timeout:12000})}
...
$("#mic").onclick=()=>{let S=speechRecognition||webkitSpeechRecognition;if(!S){say("Voice recognition is unavailable here. Try Chrome on Android.");return}rec=rec||new S;rec.lang=navigator.language||"en-US";rec.onresult=e=>{$("#cmd").value=e.results[0][0].transcript;ask($("#cmd").value)};rec.start()};
...
if("serviceWorker"in navigator)navigator.serviceWorker.register("sw.js");device();add(last,"nova");
